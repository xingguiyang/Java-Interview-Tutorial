...
# 3 方法重载
任何编程语言中都具备的一项重要特性就是名称。当你创建一个对象时，就会给此对象分配的内存空间一个名称。一个方法就是一种行为的名称。通过名称引用所各种对象，属性和方法。良好的命名可以让系统易于理解和修改。就好比写散文——目的是与读者沟通。
...
如果传入的参数类型大于方法期望接收的参数类型，你必须首先做缩窄转换，否则编译器就会报错。
